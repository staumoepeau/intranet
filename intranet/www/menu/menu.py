...
def get_context(context):
    ...
    context.top_menu = frappe.db.sql("select intranet_menu, text_style, intranet_menu_details, section_background, section_class from `tabIntranet Top Menu` where published = 1 order by order_by", as_dict=True)
    ...
    context.document_manager = frappe.db.sql("select sub_menu, file_url, file_name, heading, from `tabFile` where published = 1 and is_private == 0", as_dict=True)
