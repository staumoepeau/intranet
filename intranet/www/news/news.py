...
def get_context(context):
    context.navbar_menu = frappe.db.sql("select background_image, intranet_menu, intranet_menu_label text_style, intranet_menu_details, section_background, section_class from `tabIntranet Top Menu` where published = 1 order by order_by", as_dict=True)
